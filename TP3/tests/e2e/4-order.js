    ...
    entries: [
      {
        ...
        isValid: false,
        errorMessage: errorMessages.creditCard,
      },
    ...
    entries: [
      {
        value: "13/18",
        isValid: false,
        errorMessage: errorMessages.creditCardExpiry,
      },
      {
        value: "00/18",
        isValid: false,
...
module.exports = {
  beforeEach: function (client) {
    ...
    expectedProduct =
      productsList[utils.getRandomIntInclusive(0, productsList.length - 1)];
    client
      .url(`http://localhost:8000/${productConfig.url}${expectedProduct.id}`)
      ...
      .submitForm(productConfig.elements.form, function () {
        console.log(
          `Le produit '${expectedProduct.name}' a été ajouté au panier.`
        );
      });
  ...
  "État du panier d'achats": function (client) {
    ...
    client.assert.visible(
      shoppingCartConfig.elements.count,
  ...
  "Soumission d'un formulaire invalide": /** @param {Client} client */ function (
    client
  ) {
    // Submit the invalid form.
    client.submitForm(config.elements.form);
    // Check if the URL is the same than before.
    client.assert.urlContains(
      config.url,
      "La formulaire ne doit pas être envoyé en cas d'erreur(s)."
    );
    inputs.forEach(function (input) {
      return client.assert.containsText(
        `${input.id}-error`,
        errorMessages.required,
        `Le champ ${input.id} doit indiquer qu'il est obligatoire.`
      );
    });
  },
  "Remplissage du formulaire": /** @param {Client} client */ function (client) {
    inputs.forEach(function (input) {
      input.entries.forEach(function (entry) {
        client.clearValue(input.id).setValue(input.id, entry.value);
        client.submitForm(config.elements.form);
  ...
  "Soumission d'un formulaire valide": /** @param {Client} client */ function (
    client
  ) {
    const confirmationConfig = require("./data/config.json").confirmation;
    inputs.forEach(function (input) {
      const inputObj = input.entries.find((e) => e.isValid);
      client.clearValue(input.id).setValue(input.id, inputObj.value);
    });
    ...
    client.assert.urlContains(
      confirmationConfig.url,
      "La page de confirmation doit être affichée."
    );

    // Check if the name and the confirmation number are valid.
    client.assert
      .containsText(
        confirmationConfig.elements.name,
        "Private Ryan",
        "Le nom indiqué doit être 'Private Ryan'."
      )
      .assert.containsText(
        confirmationConfig.elements.confirmationNumber,
        "2",
        "Le numéro de confirmation doit être '2'."
      );

    // Validate if the shopping cart count is hidden when the cart is empty.
    client.assert.hidden(
      shoppingCartConfig.elements.count,
